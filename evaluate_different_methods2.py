...
if __name__ == "__main__":
    ...
    ans = []
    for mislabel_rate in [0.05, 0.025]:
        for noise_type in ["symmetric", "asymmetric"]:
            for m in args.method.split('+'):
                if m == '':
                    break
                tmp = []
                for dataset in ["Cora", "CiteSeer", "PubMed", "Computers", "Photo", "ogbn-arxiv"]:
                    y_test = get_ytest(dataset, noise_type, mislabel_rate, args.test_target)
                    F1, MCC, P = [], [], []
                    for run in range(10):
                        mislabel_result_file = 'mislabel_results/{}-test={}-{}-{}-mislabel={}-{}-epochs={}-lr={}-wd={}-exp={}'.format \
                            (m, args.test_target, dataset, args.model, mislabel_rate, noise_type, args.n_epochs, args.lr,
                        ...
                        if m == 'ours':
                            mislabel_result_file = 'mislabel_results/validl1-laplacian-test=test-MLP-{}-GCN-mislabel={}-{}-sample=0.5-k={}-epochs=200-' \
                                                   'lr=0.001-wd=0.0005-exp={}'.format(dataset, mislabel_rate, noise_type, args.k, run)
